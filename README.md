 ...
 *Take note of the VM ID.*
 ...
 This version introduces additional robustness and persistence layers, which have a small impact on system resources.
 
 ### Estimated Overhead per VM
 
 | Resource | Original | Patched (Ephemeral) | Patched (Persistent/`--rm=false`) |
 | :--- | :--- | :--- | :--- |
 | **Disk (per VM)** | 0 (Cleaned up) | 0 (Cleaned up) | **~5.1 GB** (Full copy) |
 | **RAM (Host)** | Firecracker base | + ~10 MB (Relay) | + ~10 MB (Relay) |
 | **RAM (VM)** | ~2 MB | + ~8 MB (Agent) | + ~8 MB (Agent) |
 | **CPU** | Firecracker base | Negligible | Negligible |
 
 > [!NOTE]
 > The primary impact is **Disk Space** for persistent VMs. Ephemeral sandboxes remain as lightweight as the original version.
 
 ## 7. Example: Running an Agent with Persistence
